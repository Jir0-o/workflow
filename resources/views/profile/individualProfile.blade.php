...
<div class="profile-container">
    ...
    <div class="profile-header">
        ...
        <div class="image-container">
            ...
            <input type="hidden" id="profileUserId" value="{{ $userProfile->id }}">
        </div>
        ...
        <div class="user-info">
            <h2 class="username">
                ...
                @if (Auth::user()->hasRole('Super Admin'))
                <i id="editUsername" title="Edit Username" data-user-id="{{ $userProfile->id }}" 
                    class="fas fa-edit" style="cursor: pointer; font-size: 14px;"></i>
                @endif
            
                @if ($userProfile->hasRole('Super Admin'))
                    <span class="admin"> {{ $userProfile->getRoleNames()->join(', ') }}</span>
                @else
                    <span class="user"> {{ $userProfile->getRoleNames()->join(', ') }}</span>
                @endif
            </h2>
            
            <!-- Hidden input field for username -->
            <input type="text" id="usernameInput" value="{{ $userProfile->name }}" style="display:none;">
            <input type="hidden" id="name_user_id" value="{{ $userProfile->id }}">
            ...
            <p class="stats">
                ...
                <span id="emailDisplay">{{ $userProfile->email }}</span>
                @if (Auth::user()->hasRole('Super Admin'))
                    <i id="editEmail" title="Change email address" data-user-id="{{ $userProfile->id }}" class="fas fa-edit" style="cursor: pointer; font-size: 16px;"></i>
                @endif
            </p>
            
            <!-- Hidden input field for email -->
            <input type="hidden" id="email_user_id" value="{{ $userProfile->id }}">
            <input type="text" id="emailInput" value="{{ $userProfile->email }}" style="display:none;">
    ...
    <div id="profileDetails" class="tabcontent">
        <div class="profile-details">
            ...
            <div class="section user-info">
                <h3>User Info</h3>
                @if ($userInfo)
                    <p>Joined: {{ \Carbon\Carbon::parse($userProfile->created_at)->format('d M Y') }}</p>
                ...
                <div class="button-group">
                    ...
                    @else
                    <button class="btn btn-primary edit-user-btn" data-bs-toggle="modal" data-id="{{ $userProfile->id }}" data-bs-target="#editDetailsModal">
                        Edit Details
                    </button>                    
                    @endif
                </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#imageInput').change(function() {
            ...
            let userId = $('#profileUserId').val(); // Get user ID
        ...
        $('#editUsername').click(function() {
            ...
            let userId = $(this).data('user-id'); // Get user ID from data attribute
            ...
            $('#user_id').val(userId); // Store user ID in hidden input field
        });

        // When user stops editing (on blur or enter key)
        $('#usernameInput').blur(function() {
            var newUsername = $(this).val();
            let userId = $('#name_user_id').val();
