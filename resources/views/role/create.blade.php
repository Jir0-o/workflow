        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="overflow-x-auto relative shadow-md sm:rounded-lg">
                ...
                <form action="{{ route('roles.store') }}" method="POST" class="p-4">
                    ...
                    <div class="flex gap-6 mb-6 px-6 py-4">
                        @foreach ($permissions as $item)
                        <div class="focus:outline-none text-sm leading-normal pt-2 text-gray-900 dark:text-gray-900 ">
                            <div class="ml-3 role-management-checkbox">
                                <input onclick="checksinglepermission('role-management-checkbox','management')" name="permissions[]" id="permission_{{$item->id}}" value="{{$item->id}}" type="checkbox" class="w-4 h-4 text-blue-600 bg-gray-100 rounded border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                                <label for="permission_{{$item->id}}" class="ml-2 text-lg text-gray-900 dark:text-gray-900">
                                    {{$item->name}} <br>
                                </label>
                            </div>
                        </div>
